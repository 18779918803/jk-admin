...
public class ProcessServiceImpl implements IProcessService {
        ...
        try {
            ...
            if (variables.size() > 0) {
                ...
                for (BizTodoItem update : updateList) {
                    ...
                    if (update.getTodoUserId().equals(userName)) {
                        update.setIsView("1");
                        update.setIsHandle("1");
                        update.setHandleUserId(userName);
                ...
                if (StringUtils.isNotEmpty(comment)) {
                    ...
                    identityService.setAuthenticatedUserId(sessionUser.getUsername());
                    comment = agree ? "【同意】" + comment : "【拒绝】" + comment;
                    taskService.addComment(taskId, instanceId, comment);
                } else {
                    identityService.setAuthenticatedUserId(sessionUser.getUsername());
                ...
                taskService.resolveTask(taskId, variables);
                // 只有签收任务，act_hi_taskinst 表的 assignee 字段才不为 null
                taskService.claim(taskId, sessionUser.getUsername());
                taskService.complete(taskId, variables);
                ...
                for (BizTodoItem update : updateList) {
                    // 找到当前登录用户的 todoitem，置为已办
                    if (update.getTodoUserId().equals(sessionUser.getUsername())) {
                        update.setIsView("1");
                        update.setIsHandle("1");
                        update.setHandleUserId(sessionUser.getUsername());
                        update.setHandleUserName(sessionUser.getUsername());
                        update.setHandleTime(DateUtils.getNowDate());
                        bizTodoItemService.updateBizTodoItem(update);
                    } else {
                        bizTodoItemService.deleteBizTodoItemById(update.getId()); // 删除候选用户组其他 todoitem
    ...
    public IPage<HistoricActivity> selectHistoryList(Map map) {
        ...
        list.forEach(instance -> {
            ...
            Integer userId = bizTodoItemMapper.getUserId(instance.getAssignee());
    ...
    public void delegate(String taskId, String fromUser, String delegateToUser) {
        ...
        if (updateItem != null) {
            Integer userId = bizTodoItemMapper.getUserId(delegateToUser);
            User user = userService.selectById(userId);
            updateItem.setTodoUserId(user.getUsername());
