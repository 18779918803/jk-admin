...
public class LeaveController extends BaseController {
    ...
    public DataTable list(@RequestParam Map para) {
        ...
        for (Leave leave : leaves) {
            Integer userId = bizTodoItemMapper.getUserId(leave.getCreateBy());
            User sysUser = userService.selectById(userId);
            if (sysUser != null) {
                leave.setCreateUserName(sysUser.getName());
            }
            Integer userId2 = bizTodoItemMapper.getUserId(leave.getApplyUser());
